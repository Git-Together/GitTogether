...
describe('Counter component', () => {
  xit('should display count', () => {
    const { p } = setup();
    expect(p.text()).to.match(/^1$/);
  });

  xit('first button should call increment', () => {
    const { buttons, actions } = setup();
    buttons.at(0).simulate('click');
    expect(actions.increment.called).to.be.true;
  });

  xit('second button should call decrement', () => {
    const { buttons, actions } = setup();
    buttons.at(1).simulate('click');
    expect(actions.decrement.called).to.be.true;
  });

  xit('third button should call incrementIfOdd', () => {
    const { buttons, actions } = setup();
    buttons.at(2).simulate('click');
    expect(actions.incrementIfOdd.called).to.be.true;
  });

  xit('fourth button should call incrementAsync', () => {
    const { buttons, actions } = setup();
