...
export function getRepoTree(repo) {
    return (dispatch, getState) => {
        ...
        axios.get(`https://api.github.com/repos/${repo}?access_token=${token}`)
            .then(fetched => {
                fetchedRepo = fetched
                repoId = fetchedRepo.data.id
                return axios.get(`https://api.github.com/repos/${repo}/git/refs/?access_token=${token}`)
            }).then(repoSha => {
                return axios.get(`https://api.github.com/repos/${repo}/git/trees/${repoSha.data[0].object.sha}?recursive=1&access_token=${token}`);
            }).then(tree => {
                dispatch({
            ...
            .then(() => {
                return axios.get(`https://api.github.com/repos/${repo}/collaborators?access_token=${token}`)
            }).then(collaborators => {
                dispatch({
